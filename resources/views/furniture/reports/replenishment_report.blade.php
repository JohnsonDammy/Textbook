...
@section('title', 'Replenishment Report')
@section('content')
<!-- main -->
<main>
    <div class="container">
            ...
            <div class="col-12 col-md-6 col-xl-4">
                <div class="page-titles">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><a href="javascript:void(0)">Replenishment Report</a>
                        </li>
                    </ol>
                </div>
            </div>
            <div class="offset-xl-5 text-end col-12 col-md-6 col-xl-3 mb-3">
                <div class="form-group">
                    <select class="form-control form-control-lg fs-4 wide rounded-0" id="furniture-category">
                        <option value="/reports/replenishment" selected>Replenishment Report</option>
        ...
        <div class="row justify-content-center align-items-center">
            ...
            <div class="col-12 col-md-9 my-3">
                <form class="row  g-4" action="/reports/replenishment/search">
                    ...
                    @endif
                    <div class="col-12 col-md-6 col-xl-4">
                        <select class="form-select form-control rounded-0" name="replenishment_status">
                            <option selected value="">Replenishment Status</option>
                            @foreach (getreplenishmentstatus() as $repstatus)
                            <option value="{{ $repstatus->id }}">{{ $repstatus->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-12 col-md-6 col-xl-4">
                        <select class="form-select form-control rounded-0" name="item_id">
                            <option selected value="">Item Description</option>
                            @foreach (getListOfAllItems() as $item)
                            <option value="{{ $item->id }}">{{ $item->getCategory->name }} - {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-12 col-md-6 col-xl-4">
            ...
            <div class="col-12 col-md-3 my-3">
                ...
                <div class="col-12 mt-4 d-flex align-items-center justify-content-center">
                    <a href="/reports/replenishment" class="px-4 fs-3 btn-reset text-decoration-underline">Reset</a>
                </div>
            </div>
            </form>
        </div>
                ...
                <table class="table table-nnowrap">
                    <thead>
                        <tr>
                            ...
                            <th>Replenishment Requested Count</th>
                            <th>Approved Replenishment Count</th>
                            <th>Rejected Replenishment Count</th>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $item['replenishment_count'] }}</td>
                            <td>{{ $item['approved_replenished_count'] }}</td>
                            <td>{{ $item['rejected_replenished_count'] }}</td>
                            <td>{{ $item['replenishment_status'] }}</td>
...
<script>
    ...
    $('#download_report').on('click', function() {
        ...
        var replenishment_status = url.searchParams.get("replenishment_status");
        var start_date = url.searchParams.get("start_date");
        var end_date = url.searchParams.get("end_date");
        $.ajax({
            url: "/reports/replenishment/download",
            type: "get",
            data: {
            ...
            success: function(response) {
                ...
                link.download = `Replenishment Report.xls`;
                link.click();
                console.log('success')
            },
            error: function(err) {
