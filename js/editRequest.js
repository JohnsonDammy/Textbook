...
function brokenItemAppend() {
    if (broken_item_array.length > 0) {
        ...
        $('#broken-item-table').html(`
                            <tr>
                                <td colspan="4" align = "center">Damaged items not added</td>
                            </tr>`)
    }
}
...
function saveBrokenItems() {
    ...
    if (broken_item_array.length > 0 && furn_count_input > 0) {
        ...
        window.localStorage.setItem(`editBrokenItems${collection_id}`, JSON.stringify(broken_item_array));
        window.localStorage.setItem(`editTotalCount${collection_id}`, furn_count_input);
        $('#broken_array').val(JSON.stringify(broken_item_array));
        console.log(JSON.parse(window.localStorage.getItem(`editBrokenItems${collection_id}`)));
        // $('#collection-submit-btn').removeClass("disabled");
        console.log($('#broken_array').val());
        $("#listsuccess").click()
    } else {
        if (broken_item_array.length <= 0) {
            window.localStorage.removeItem(`editBrokenItems${collection_id}`)
            broken_item_array = []
        }
        if (furn_count_input <= 0)
            window.localStorage.removeItem(`editTotalCount${collection_id}`)
...
function updateBrokenItem(index) {
    ...
    console.log("CAT", cat_id, "ITEM", item_id, "COUNT", count)
...
$("#success-pop-ok").click(function() {
 ...
 $('#collection-submit-btn').click(function() {
    ...
    if (broken_item_array.length > 0) {
        ...
        $.ajax({
            url: `/manage-requests/${collection_id}`,
            type: "PUT",
            data: {
                _token: csrf_token,
