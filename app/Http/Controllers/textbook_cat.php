...
class textbook_cat extends Controller
{
    ...
    public function saveCheckedItems(Request $request)
    {
        ...
        if (session('quoteStatus') != "Quote Created") {
            ...
            if ($newItems != null) {
                ...
                $selectedRecords = textbookCatModel::whereIn('id', $newItems)->get(['id', 'Title', 'ISBN', 'Price', 'Grade']);
            ...
            if ($isNextClicked) {
                ...
                $nextPageUrl = route('filterTextbook', [
                    'page' => $nextPage,
                    // Assuming $textbooksData is a paginator
    ...
    public function submitSavedItems(Request $request)
    {
        ...
        if($orderedAmount > $AllocatedTextbook){
            return redirect()->back()->with(['successD' => 'Order amount is greater than Allocation funds amount, please reduce your quantity']);
        }else{
            try {
