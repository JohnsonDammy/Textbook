...
class Receive_Quote extends Controller
{
    public function index($requestType, Request $request)
    {
        ...
        // $statusRequestNew = inbox_school::where('requestType', session("requestType"))->where('school_emis', session("emis"))->value('ReworkComment');

        // if ($statusRequestNew == "Declined") {
        //     $statusComment = RequestFundsModel::where('id', $fundsId)->value("comment");
        // } else {
        //     $statusComment = "";
        // }
        ...
        $statusRequestNew = inbox_school::where('requestType', session("requestType"))
    ->where('school_emis', session("emis"))
    ->value('ReworkComment');

        if ($statusRequestNew === null) {

            //echo "The value is null!";
            $statusCommentNew = "";

        } else {
            // The value is not null, you can use it
            $statusCommentNew = $statusRequestNew;
        }

        session(['statusCommentNew' => $statusCommentNew]);
        ...
        $DeviationReason = ef48docs::where('school_emis', $emis)->where('requestType', session('requestType'))->value('deviationReason');
    ...
    //Download EF58 
    public function downloadEF58New()
    {
        // $emis = auth()->user()->username;
        // $requestType = session('requestType', 'default_value_if_not_set');
    ...
    public function updateRecommended(Request $request)
    {
        ...
        if ($isRecommendedClicked == "true") {
            ...
            $deviationReason = $request->input("deviationReason");
    ...
    public function updateDeviationReason(Request $request ){
         $DeviationReason = $request->input('DeviationR');
            ...
            DB::table('ef48docs')
                ...
                return response()->json(['message' => 'Deviation updated successfully']);
